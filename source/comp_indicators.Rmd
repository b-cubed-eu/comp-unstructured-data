...
```{r}
occ_download_get(birdcubeflanders_year)
```
...
```{r}
abv <- process_cube(abv_data_total |>
                      select(TAG, Shape_Area, Shape_Leng, mgrsCode,
                             year, speciesKey, species, family,
                             individualCount, minCoordinateUncertaintyInMeters =
                               coordinateUncertaintyInMeters),
                    cols_occurrences = "individualCount")
```
...
williams_evenness_map_birdcube <- williams_evenness_map(birdcube, cell_size = 10)
...
tax_distinct_map_abv <- tax_distinct_ts(abv, cell_size = 10)
plot(tax_distinct_map_abv)
```

### Time series

```{r taxonomic distinctness time series, fig.show="hold", out.width="50%"}
tax_distinct_ts_birdcube <- tax_distinct_ts(birdcube)
plot(tax_distinct_ts_birdcube)

tax_distinct_ts_abv <- tax_distinct_ts(abv)
...
```{r species richness map, fig.show="hold", out.width="50%"}
hill0_map_birdcube <- hill0_map(birdcube, cell_size = 10)
plot(hill0_map_birdcube)

hill0_map_abv <- hill0_ts(abv, cell_size = 10)
plot(hill0_map_abv)
```
...
```{r hill-shannnon diversity map, fig.show="hold", out.width="50%"}
hill1_map_birdcube <- hill1_map(birdcube, cell_size = 10)
plot(hill1_map_birdcube)

hill1_map_abv <- hill1_ts(abv, cell_size = 10)
plot(hill1_map_abv)
```
...
```{r hill-simpson diversity map, fig.show="hold", out.width="50%"}
hill2_map_birdcube <- hill2_map(birdcube, cell_size = 10)
plot(hill2_map_birdcube)

hill2_map_abv <- hill2_ts(abv, cell_size = 10)
plot(hill2_map_abv)
```
...
To compare these indicators we selected three species: the extremly common ..., generalist ..., the rare species ... (werken met tabs om het overzichtelijk te houden)
