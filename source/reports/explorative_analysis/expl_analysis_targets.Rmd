...
```{r}
abv <- tar_read(abv_data)
cube <- tar_read(birdcube_data)
```
...
```{r}
cube |>
  filter(mincoordinateuncertaintyinmeters < 5000) |>
...
tar_read(trend_comp_3) |>
  filter(trend_comp_per == "cyclus") |>
  filter(category %in% c("Common", "Very common", "Extremly common"),
         correlation < 0.5) |> 
  knitr::kable()
```

#### Lowest correlations {.tabset}

##### **Sylvia curruca**

```{r}
tar_read(filter3_abv) |>
  ggplot(aes(x = cyclus))
```
