...
```{r}
tar_read(data) |> 
  filter(species == "Motacilla flava",
         id_spat_res == "1km") |>
  group_by(species, id_dataset, year) |>
  summarise(n = sum(n)) |>
  ggplot(aes(x = year, y = n, color = id_dataset)) +
  geom_point() +
  geom_line()
```
...
```{r}
datatable(trend_comp_2_cyclus)
```

```{r}
shared_filter2_cyclus <- SharedData$new(tar_read(filter2) |> 
                                          filter(id_spat_res == "1km",
