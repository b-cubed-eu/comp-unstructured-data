...
p3 + labs(title = paste0(p3$labels$title, " - Cube filtered")) +
  theme_bw(base_size = 12)
```
...
# Calculate relative occurences to the total occurences of the species order
rel_occ <- spec_occ_ts_cube$data %>%
  dplyr::filter(orderKey ==  spec_occ_ts_cube$data %>%
                  dplyr::filter(scientificName == spec) %>%
                  dplyr::first() %>%
                  dplyr::pull(orderKey)) %>%
  dplyr::group_by(year) %>%
  dplyr::summarise(tot = sum(diversity_val)) %>%
  dplyr::left_join(spec_occ_ts_cube$data %>%
                     dplyr::filter(scientificName == spec),
                   by = "year") %>%
  dplyr::mutate(rel_occ = diversity_val / tot)

p3 <- ggplot(rel_occ, aes(x = year, y = rel_occ)) + geom_point() +
...
# Save
ggsave(
  file.path(out_path, paste0(spec, "_rel_occ_ts.png")),
  p3,
  width = 10, height = 4, dpi = 300
)
```
