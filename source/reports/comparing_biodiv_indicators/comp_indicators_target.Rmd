...
# class(proc_cube_abv)
# class(proc_cube_unstr)

# number of species in each dataset is very different 665 (unstructered) 
# and 180 (abv).
```
...
#saveRDS(hill0_ts_abv, file = "hill0_ts_abv.RDS")
hill0_ts_abv <- readRDS(file = "hill0_ts_abv.RDS")
...
#saveRDS(hill0_ts_unstr, file = "hill0_ts_unstr.RDS")
hill0_ts_unstr <- readRDS(file = "hill0_ts_unstr.RDS")
...
#saveRDS(hill0_ts_unstr_filt, file = "hill0_ts_unstr_filt.RDS")
hill0_ts_unstr_filt <- readRDS(file = "hill0_ts_unstr_filt.RDS")
...
Three plots combined in one plot
...
dfplot_hill0 <- dplyr::bind_rows(list(obs_richness_ts_abv$data,
                                 ...
                                 .id = "id") %>%
  dplyr::left_join(data.frame(id = c("1", "2", "3", "4", "5"),
                              type = c(rep("obs", 2), rep("est", 3)),
                              dataset = c("abv", "unstr.", "abv", "unstr.", "unstr.")),
                   by = "id")
...
As expected, the estimated species richness for the abv dataset does not vary
in time, as the survey effort is constant and the observed species richness does
not vary a lot.
