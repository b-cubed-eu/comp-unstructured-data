...
```{r}
# to be included in the target workflow
# hill0_ts_abv <- b3gbi::hill0_ts(data = proc_cube_abv,
#                                 coverage = 0.95,
#                                 cutoff_length = 5,
#                                 ci_type = "none")
...
```{r}
# hill0_ts_unstr <- b3gbi::hill0_ts(data = proc_cube_unstr,
#                                 coverage = 0.95,
#                                 cutoff_length = 5,
#                                 ci_type = "none")

#saveRDS(hill0_ts_unstr, file = file.path(interim_path, "hill0_ts_unstr.RDS"))
hill0_ts_unstr <- readRDS(file = file.path(interim_path, "hill0_ts_unstr.RDS"))

p2 <- plot(hill0_ts_unstr)
p2 + labs(title = paste0(p1$labels$title, " - Cube")) +
  theme_bw(base_size = 12)
```

### Hill0 - estimated species richness unstructered data (filtered)
```{r}
# unique taxonkeys in abv
taxon_abv <- unique(proc_cube_abv$data$taxonKey)

# filter abv species from unstructered data
proc_cube_unstr_filt <- proc_cube_unstr
proc_cube_unstr_filt$data <- proc_cube_unstr_filt$data %>%
  dplyr::filter(taxonKey %in% taxon_abv & year < 2019) %>%
  dplyr::slice_sample(n = 100000)   # for testing to be removed!!

# hill0_ts_unstr_filt <- b3gbi::hill0_ts(data = proc_cube_unstr_filt,
#                                 coverage = 0.95,
#                                 cutoff_length = 5,
#                                 ci_type = "none")

# saveRDS(hill0_ts_unstr_filt, file = file.path(interim_path,
#                                                "hill0_ts_unstr_filt.RDS"))
hill0_ts_unstr_filt <- readRDS(file = file.path(interim_path,
                                                "hill0_ts_unstr_filt.RDS"))

p3 <- plot(hill0_ts_unstr_filt)
p3 + labs(title = paste0(p1$labels$title, " - Cube filtered")) +
  theme_bw(base_size = 12)
```


```{r}
# observed species richness - with filtering of abv species

obs_richness_ts_cube_filt <- b3gbi::obs_richness_ts(proc_cube_unstr_filt) 

plot(obs_richness_ts_cube_filt)
```

Plots combined in one plot with facet_wrap

```{r}
dfplot_hill0 <- dplyr::bind_rows(list(obs_richness_ts_abv$data,
                                      obs_richness_ts_cube$data,
                                      obs_richness_ts_cube_filt$data,
                                      hill0_ts_abv$data,
                                      hill0_ts_unstr$data,
                                      hill0_ts_unstr_filt$data),
                                 .id = "id") %>%
  dplyr::left_join(data.frame(id = as.character(c(1:6)),
                              type = factor(c(rep("obs", 3), rep("est", 3)), 
                                               c("obs", "est")),
                              dataset = c(rep(c("abv", "unstr.", "unstr. filt."), 2))),
                   by = "id")
...
The estimated diversity corrects for this with the rarification algoritm
...
```{r}
# Load general data for all species
...
tot_occ_order <- spec_occ_ts_cube$data %>%
  dplyr::left_join(order_list,
                   by = c("taxonKey" = "key")) %>%
  dplyr::filter(orderKey == 729) %>%
  dplyr::group_by(year) %>%
  dplyr::summarise(tot = sum(diversity_val))

rel_occ <- spec_occ_ts_cube$data %>%
  dplyr::filter(scientificName == "Cettia cetti" & year < 2019) %>%
  dplyr::left_join(tot_occ_order, by = "year") %>%
  dplyr::mutate(rel_occ = diversity_val / tot)

ggplot(rel_occ, aes(x = year, y = rel_occ)) + geom_point() +
...
rel_occ <- spec_occ_ts_cube$data %>%
  dplyr::filter(scientificName == "Passer montanus" & year < 2019) %>%
  dplyr::left_join(tot_occ_order, by = "year") %>%
  dplyr::mutate(rel_occ = diversity_val / tot)
...
rel_occ <- spec_occ_ts_cube$data %>%
  dplyr::filter(scientificName == "Luscinia megarhynchos" & year < 2019) %>%
  dplyr::left_join(tot_occ_order, by = "year") %>%
  dplyr::mutate(rel_occ = diversity_val / tot)
