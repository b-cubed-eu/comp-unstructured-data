...
dfplot_hill0 <- dplyr::bind_rows(list(obs_richness_ts_abv$data,
                                      ...
                                      hill0_ts_unstr$data),
                                 .id = "id") %>%
  dplyr::left_join(data.frame(id = as.character(c(1:4)),
                              type = factor(c(rep("obs", 2), rep("est", 2)), 
                                               c("obs", "est")),
...
df_rel_occ <- purrr::map_dfr(spec_vec,
...
p3 <- df_rel_occ %>%
  dplyr::filter(scientificName == spec) %>%
  ggplot(aes(x = year, y = rel_occ)) + geom_point() +
  geom_smooth(se = FALSE) +
  theme_bw(base_size = 12) +
  scale_y_continuous(limits = c(0, 0.02)) +
...
p1 <- spec_occ_ts_abv$data %>%
  ...
  geom_point() + geom_smooth(se = FALSE, span = 2) +
...
ggsave(file.path(out_path, "spieces_rel_occurences_cube.png"), p1,
  width = 10, height = 4, dpi = 300)
...
df_plot <- dplyr::bind_rows(spec_occ_ts_abv$data %>%
                             dplyr::select(year, 
                                           scientificName,
                                           occurences = diversity_val),
  ...
  dplyr::left_join(data.frame(id = as.character(c(1:3)),
                              dataset = c("ABV", "unstr.", "unstr. norm")),
                   by = "id")
