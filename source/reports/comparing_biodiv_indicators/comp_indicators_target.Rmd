...
p1 <- spec_occ_ts_abv$data %>%
  ...
  geom_smooth(se = FALSE, span = 2, colour = alpha("blue", 0.5)) +
  geom_point(colour = "darkorange", size = 2) +
...
dataset_labs <- c(
  "ABV" = "Occurences (ABV)",
  "unstr." = "Occurences (unstructured)",
  "unstr. freq." = "Relative frequency (unstructured)"
)
...
p_grid <- ggplot(df_plot, aes(x = year, y = occurences)) +
  ...
  labs(x = "", y = NULL) +   # remove global y label
  theme_bw(base_size = 12) +
  facet_grid(
    dataset ~ scientificName,
    scales = "free_y",
    switch = "y",                         # move strip to left
    labeller = labeller(dataset = dataset_labs)
  ) +
  theme(
