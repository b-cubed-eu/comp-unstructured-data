...
query_abv <- "SELECT
  ...
  IF(ISNULL(\"order\"), NULL, SUM(COUNT(*)) OVER (PARTITION BY \"order\", \"year\")) AS orderCount,
  IF(ISNULL(family), NULL, SUM(COUNT(*)) OVER (PARTITION BY family, \"year\")) AS familyCount,
  IF(ISNULL(genus), NULL, SUM(COUNT(*)) OVER (PARTITION BY genus, \"year\")) AS genusCount
  FROM
  occurrence
  WHERE
...
write_csv(birdcube_data_out,
          file.path(out_path, "birdflanders_cube_1km_temp.csv"))

## Spatial object
write_sf(birdcube_data_out_sf,
         file.path(out_path, "birdflanders_cube_1km_temp.gpkg"))
```
