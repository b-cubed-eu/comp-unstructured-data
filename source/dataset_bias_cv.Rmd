...
prevalence_df %>%
  ggplot(aes(x = abv, y = birdcube)) +
    ...
    geom_point(aes(shape = rarity), size = 2, colour = "cornflowerblue") +
...
remotes::install_github("b-cubed-eu/dubicube#25")
...
We visualise the root mean squared error (RMSE) per species. Labels are included for species with values above the 95% quantile of.

```{r}
prevalence_cv %>%
  distinct(species, rarity, rmse) %>%
  left_join(prevalence_df, by = join_by(species, rarity)) %>%
  mutate(rmse_95 = quantile(rmse, probs = 0.95)) %>%
  ggplot(aes(x = abv, y = birdcube)) +
    geom_smooth(method = "lm", formula = "y ~ log(x)", colour = "firebrick") +
    geom_point(aes(shape = rarity, colour = rmse), size = 2) +
    labs(x = "Proportion of occupied grid cells\nin ABV dataset",
         y = "Proportion of occupied grid cells\nin cube dataset",
         shape = "Rarity", colour = "RMSE") +
...
prevalence_cv %>%
  ...
  ggplot(aes(x = abv, y = birdcube)) +
    geom_smooth(method = "lm", formula = "y ~ log(x)", colour = "firebrick") +
    geom_point(aes(shape = rarity, colour = rmse), size = 2) +
    geom_text(aes(label = ifelse(rmse > rmse_95,
                                 paste0(species, "\n(value: ",
                                        round(rmse, 3), ")"),
                                 NA)
                  ),
              hjust = 1, vjust = -0.5, size = 3, check_overlap = TRUE) +
    labs(x = "Proportion of occupied grid cells\nin ABV dataset",
         y = "Proportion of occupied grid cells\nin cube dataset",
         shape = "Rarity", colour = "RMSE") +
    scale_colour_viridis_c(option = "turbo") +
...
We visualise the maximum error per species. Labels are included for species with values above the 95% quantile of.
...
prevalence_cv %>%
  summarise(max_error = max(error),
            .by = c("species", "rarity")) %>%
  left_join(prevalence_df, by = join_by(species, rarity)) %>%
  mutate(max_error_95 = quantile(max_error, probs = 0.95)) %>%
  ggplot(aes(x = abv, y = birdcube)) +
    geom_smooth(method = "lm", formula = "y ~ log(x)", colour = "firebrick") +
    geom_point(aes(shape = rarity, colour = max_error), size = 2) +
    geom_text(aes(label = ifelse(max_error > max_error_95,
                                 paste0(species, "\n(value: ",
                                        round(max_error, 3), ")"),
                                 NA)
                  ),
