...
prevalence_df %>%
  ggplot(aes(x = abv, y = birdcube)) +
    geom_abline(slope = 1, intercept = 0, colour = "firebrick",
                linewidth = 1) +
    annotate("label", x = 0.75, y = 0.6,
             label = "Higher prevalence in ABV",
             color = "black") +
    annotate("label", x = 0.45, y = 0.6,
...
We look at the `r (1 - quantile) * 100` % species with the highest maximum error.
...
```{r, warning=FALSE, out.width="90%", message=FALSE}
# Calculate minimum error per species
prevalence_cv %>%
  mutate(min_error = abs(min(error)),
         .by = c("species", "rarity")) %>%
  plot_cross_validation(
    prevalence_df,
    measure = "min_error",
...
We look at the `r (1 - quantile) * 100` % species with the highest minimum error (absolute value).

```{r}
top_spec_min_error_df <- prevalence_cv %>%
  mutate(min_error = abs(min(error)),
         .by = c("species", "rarity")) %>%
  distinct(species, rarity, min_error) %>%
  slice_max(min_error, prop = 1 - quantile) %>%
  arrange(desc(min_error))

top_spec_min_error_df %>%
  knitr::kable(digits = 5)
```

```{r}
top_min_error_specs <- top_spec_min_error_df %>%
  slice_max(min_error, n = 5) %>%
  pull(species)

birdcube_dataset_filtered %>%
  dplyr::filter(species %in% top_min_error_specs) %>%
  count(species, datasetname) %>%
  left_join(top_spec_min_error_df, by = join_by(species)) %>%
  mutate(species = reorder(species, min_error, decreasing = TRUE)) %>%
  mutate(datasetname = tidytext::reorder_within(datasetname, n, species)) %>%
  ggplot(aes(x = datasetname, y = n)) +
    geom_bar(stat = "identity") +
    geom_text(aes(label =  n), vjust = 0.3, hjust = -0.3, size = 3) +
