...
```{r, message=FALSE}
abv_data_raw <- read_csv(
  here::here("data", "interim", "abv_data.csv"),
  show_col_types = FALSE)

# Cut rarity
abv_data <- abv_data_raw[-1] %>%
  dplyr::select("mgrscode", "year", "specieskey", "species", "family", "n",
              "mincoordinateuncertaintyinmeters", "familycount") %>%
  mutate(n_obs = n(),
         .by = "species") %>%
  mutate(rarity = cut(
...
birdcube_filtered <- birdcube_dataset_filtered %>%
  group_by(mgrscode, year, specieskey, species, family, familycount) %>%
  summarise(
    n = sum(n),
...
- calculate indicator on abv_species and birdcube_filtered
