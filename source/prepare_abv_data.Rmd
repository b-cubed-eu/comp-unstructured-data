...
```{r}
utm_grid <- read_sf(file.path(data_path, "utm_grid", "utm1_vl_zBRU.shp"))
utm_grid <- utm_grid %>%
...
We add the geometry to the data layers.
For some reason, one grid cell from ABV is located outside Flanders (in Brussels).

```{r}
sampling_framework_abv %>%
  anti_join(utm_grid, by = join_by(mgrsCode))
```

We only keep the Flemish grid cells. 31UES9939 is removed.

```{r}
abv_data_total_sf <- utm_grid %>%
  left_join(abv_data_total, utm_grid, by = join_by(mgrsCode)) %>%
  st_sf(sf_column_name = "geometry")

sampling_framework_abv_sf <- utm_grid %>%
  left_join(sampling_framework_abv, utm_grid, by = join_by(mgrsCode)) %>%
  st_sf(sf_column_name = "geometry")
```

```{r}
ggplot() +
  geom_sf(data = sampling_framework_abv_sf, aes(fill = Stratum)) +
